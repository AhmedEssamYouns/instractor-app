...
const StudentQuizList = () => {
        ...
        const unsubscribeGrades = onSnapshot(collection(db, 'grades'), (gradeSnapshot) => {
            ...
            gradeSnapshot.docs.forEach(gradeDoc => {
                ...
                if (gradeData.userId === FIREBASE_AUTH.currentUser.uid) {
                    gradesObj[gradeDoc.id.split('_')[0]] = gradeData;
                }
            });
            setGrades(gradesObj);
            setLoading(false);
    ...
    const handleQuizPress = (quizId) => {
        Alert.alert(
            translations.confirmAccessTitle,
            translations.confirmAccessMessage,
            [
                {
                ...
                },
                {
                    ...
                    onPress: () => navigation.navigate('QuizAttempt', { quizId }),
                },
            ]
        );
    };
